...
const OTPVerificationModal = ({ isOpen, toggle, email, onVerified }) => {
  const { confirmVerification, resendVerificationEmail } = useContext(AuthContext);
  ...
  // Handle input focus when modal opens
  useEffect(() => {
    if (isOpen) {
      // Focus the input field after a brief delay to ensure modal is rendered
      setTimeout(() => {
  ...
  const handleResendVerification = async () => {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        throw new Error(result?.error?.response?.data?.message || "Failed to resend verification email");
      }
    } catch (err) {
